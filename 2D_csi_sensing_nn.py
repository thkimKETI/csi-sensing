...
# MQTT Library
from mqtt_config_2D import BROKER_ADDRESS, PORT, TOPIC, MAC_ADDRESS, create_mqtt_message
...
inference_queue = Queue(maxsize=100)  # 추론을 위한 데이터 큐
storage_queue = Queue(maxsize=100) # 데이터를 저장하기 위한 큐 
visualization_queues = [Queue(maxsize=100) for _ in range(4)]  # 시각화를 위한 데이터 큐
...
def get_amplitude(csi, visual, is_sequence=False):
    ...
    if not visual:
        ...
        amplitude = np.concatenate((amplitude[127:184],amplitude[66:123]), axis=0)    
...
def apply_chunk_butterworth_and_rpca(data_chunk, fs=5, cutoff=0.4, order=1, rpca_lamb=None):
    """
        ...
        try:
            ...
            for entry in entries:
                ...
                if ts_second != mac_current_second[mac_idx]:
                    ...
                    print(f"csi per Second[{mac_idx}] - 초당 데이터 개수: {mac_second_count[mac_idx]} - esp_time: {timestamp}") # terminal Notice
...
def neural_network_inference_process(inference_queue, storage_queue, labels_dict, exit_flag, acq_bool):
    ...
    # 모델 로딩 (Load Model)
    device = "cuda"  # 필요시 CUDA 사용 가능
    save_cnt = 0 # save data counting
    activity_buffer = [] # shape이 4인 활동량 버퍼

    try:
        model_loc = TinyCSINet(num_esp=2, num_classes=2).to(device)
        state_dict = torch.load("./loc_2D_cnn_0729.pth", map_location=device) 
        model_loc.load_state_dict(state_dict)
        model_loc.eval()
        
        
        model_act = TinyTransformerNet(num_esp=2, num_classes=4).to(device)
        state_dict = torch.load("./act_2D_transformer_0729.pth", map_location=device) 
        model_act.load_state_dict(state_dict)
        model_act.eval()
    ...
    while not exit_flag.value:
        ...
        try:
            ...
            try:
                port_num, data = inference_queue.get()
                inference_data[port_num] = data

                s_port_num, s_ts, s_data = storage_queue.get()
            ...
            if all(data is not None for data in inference_data.values()) and isPushedBtn.value and inf_flag:
                ...
                if acq_bool:
                    ...
                    # ❗ 먼저 None 데이터가 있는지 확인
                        ...
                        if missing_ports:
                            ...
                            save_inf_data = np.concatenate([inference_data[i] for i in range(mac_cnt)], axis=0)

                            if save_cnt <= 540 and save_cnt > 10: 
                                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                                os.makedirs(f"/csi/datasets/mqtt/Capture/{csi_dir}", exist_ok=True)
                                filename = f"/csi/datasets/mqtt/Capture/{csi_dir}/{timestamp}_{save_cnt-10}_amp.csv"
                                np.savetxt(filename, save_data, delimiter=",")
                                os.makedirs(f"/csi/datasets/mqtt/others/{today}/{csi_dir}", exist_ok=True)
                                filename_i = f"/csi/datasets/mqtt/others/{today}/{csi_dir}/{timestamp}_{save_cnt-10}_train.csv"
                                np.savetxt(filename_i, save_inf_data, delimiter=",")
                                save_cnt += 1
                ...
                combined_data = np.stack([inference_data[i] for i in range(mac_cnt)], axis=0)
                # print("==== 추론 직전 각 MAC별 타임스탬프 ====") # terminal Notice
                ...
                with torch.no_grad():
                    # 병렬 추론을 위한 스레드 생성
                    loc_result = [None, None]
                    act_result = [None, None]

                    def infer_loc():
                        input_data = tensor_data[:, :2, :, :]
                        logits = model_loc(input_data)
                        preds = torch.argmax(logits, dim=1).cpu().numpy()
                        confs = torch.nn.functional.softmax(logits).cpu().detach().numpy()

                        loc_result[0] = preds
                        loc_result[1] = round(confs[0][0],2), round(confs[0][1], 2)

                    
                    def infer_act():
                        input_data = tensor_data[:, :2, :, :]
                        logits = model_act(input_data)
                        preds = torch.argmax(logits, dim=1).cpu().numpy() 
                        confs = torch.nn.functional.softmax(logits).cpu().detach().numpy()
                        act_result[0] = preds
                        act_result[1] = round(confs[0][0],2), round(confs[0][1], 2), round(confs[0][2],2), round(confs[0][3],2)
                ...
                print(f"[🏃] 추론 결과에 따른 활동량 :{activity_ratio:.5f}") # terminal Notice
                ...
                if len(activity_buffer) > 10:
                    ...
                    # print(f"[🏃] 10초간 평균 활동량 :{avg_per_port_10s:.5f}") # terminal Notice
                    ...
                    # 신경망 추론 결과로부터 클래스 매핑
                        ...
                        labels_dict["loc"] = f"{loc_classes[loc_result[0].item()]} {loc_result[1]}"
                        labels_dict["act"] = f"{act_classes[act_result[0].item()]} {act_result[1]}"
                else:
...
class CSIDataGraphicalWindow(QMainWindow):
    def __init__(self, labels_dict, isPushedBtn):
        ...
        self.setWindowTitle("MQTT CSI SENSING - NN 2Devices")
