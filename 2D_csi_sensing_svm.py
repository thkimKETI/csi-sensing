...
def neural_network_inference_process(inference_queue, storage_queue, labels_dict, exit_flag, acq_bool):
    ...
    try:
        model_loc = load_model("./loc_2D_svm_0729.pkl", n_classes=2, model_type="SVM")
        model_act = load_model("./act_2D_svm_0729.pkl", n_classes=4, model_type="SVM")
        print("[✅] 모델 로딩 성공")
    except Exception as e:
        print(f"[❌] 모델 로딩 실패: {e}")
        return
    ...
    while not exit_flag.value:
        ...
        try:
            ...
            if all(data is not None for data in inference_data.values()) and isPushedBtn.value and inf_flag:
                    ...
                    # ❗ 먼저 None 데이터가 있는지 확인
                        ...
                        if missing_ports:
                            ...
                            if save_cnt <= 540 and save_cnt > 10: 
                                ...
                                os.makedirs(f"/csi/datasets/mqtt/{today}/RAW/{csi_dir}", exist_ok=True)
                                filename = f"/csi/datasets/mqtt/{today}/RAW/{csi_dir}/{timestamp}_{save_cnt-10}_amp.csv"
                                np.savetxt(filename, save_data, delimiter=",")
                                os.makedirs(f"/csi/datasets/mqtt/{today}/TRAIN/{csi_dir}", exist_ok=True)
                                filename_i = f"/csi/datasets/mqtt/{today}/TRAIN/{csi_dir}/{timestamp}_{save_cnt-10}_train.csv"
                ...
                with torch.no_grad():
                    # 병렬 추론을 위한 스레드 생성
                    loc_result = [None, None, None]
                    act_result = [None, None, None]

                    def infer_loc():
                        input_tensor = tensor_data.reshape(-1, 114)
                        zones = input_tensor.numpy()
                        preds = []
                        confidences = []


                        X = zones.flatten().reshape(1, -1)
                        try:
                            probs = model_loc.predict_proba(X)[0]  # (num_classes,)
                            pred = np.argmax(probs)
                            confidence = list(probs)
                        except AttributeError:
                            pred = model_loc.predict(X)[0]
                            confidence = None  # 확률 제공 불가 시 None

                        preds.append(pred)
                        confidences.extend([round(confidence[0],2), round(confidence[1],2)])
                        ...
                        loc_result[0] = closeness
                        loc_result[1] = confidences
                    ...
                    def infer_act():
                        ...
                        X = zones.flatten().reshape(1, -1) # (1, 20520)
                        try:
                            probs = model_act.predict_proba(X)[0]  # (num_classes,)
                            pred = np.argmax(probs)
                            confidence = list(probs)
                        except AttributeError:
                            pred = model_act.predict(X)[0]
                            confidence = None  # 확률 제공 불가 시 None


                        preds.append(pred)
                        confidences.extend([round(confidence[0],2), round(confidence[1],2), round(confidence[2],2), round(confidence[3],2)])

                        if preds[0]== 0:
                            act_result[0] = "Exrecise"
                        elif preds[0]== 1:
                            act_result[0] = "Sit"
                        elif preds[0]== 2:
                            act_result[0] = "Stand"
                        else : 
                            act_result[0] = "Walking"


                        act_result[1] = confidences  # confidence 목록 반환
                ...
                if len(activity_buffer) > 10:
                    ...
                    # 신경망 추론 결과로부터 클래스 매핑
                        ...
                        labels_dict["loc"] = f"{loc_result[0]} {loc_result[1]}"
                        labels_dict["act"] = f"{act_result[0]}, {act_result[1]}"
                else:
...
class CSIDataGraphicalWindow(QMainWindow):
    def __init__(self, labels_dict, isPushedBtn):
        ...
        self.setWindowTitle("MQTT CSI SENSING - ML 2Devices")
