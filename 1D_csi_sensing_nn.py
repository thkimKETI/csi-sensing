...
# MQTT Library
from mqtt_config_1D import BROKER_ADDRESS, PORT, TOPIC, MAC_ADDRESS, create_mqtt_message
...
def neural_network_inference_process(inference_queue, storage_queue, labels_dict, exit_flag, acq_bool):
    ...
    try:
        model_loc = TinyCSINet(num_esp=1, num_classes=2).to(device)
        state_dict = torch.load("./loc_1D_cnn_0729.pth", map_location=device) 
        model_loc.load_state_dict(state_dict)
        model_loc.eval()
        
        
        model_act = TinyTransformerNet(num_esp=1, num_classes=4).to(device)
        state_dict = torch.load("./act_1D_transformer_0729.pth", map_location=device) 
...
class CSIDataGraphicalWindow(QMainWindow):
    def __init__(self, labels_dict, isPushedBtn):
        ...
        self.setWindowTitle("MQTT CSI SENSING- NN 1Device")
